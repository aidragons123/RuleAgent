...
class Pipeline:
    ...
    def validate_source(
        self, source: SourceFile, vectors: list[TestVector],
        run_id: str = "revalidate", evidence_out_path: Path | None = None,
    ) -> PipelineResult:
        """Re-run steps 5-9 (differential -> diagnosis -> traceability ->
        evidence) against an already-produced SourceFile and vector set,
        skipping test synthesis and implementation generation (steps 2 and
        4). Used by the UI's AI bug-injection flow: take a prior clean
        run's generated code and vectors, mutate the code, and re-validate
        without regenerating or re-synthesising anything, so the only
        variable between the two reports is the one deliberately-introduced
        change."""
        cfg = self.cfg
        tracer = Tracer(
            case_ids=[run_id], model=cfg["llm"]["model"],
        ...
        out_path.parent.mkdir(exist_ok=True, parents=True)
        out_path.write_text(source.content)
        tracer.record_step("Use provided implementation", "ai", detail=source.path)
